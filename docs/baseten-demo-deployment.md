...
> diagnosis or treatment behavior, and no clinical-use claim. This slice
> deploys **nothing** live; it delivers the server-side backend, the
> deployment definition, and instrumentation for the follow-up execution step.

This is the hosting layer for an on-demand Baseten demo of the clinical-intake
prototype: Qwen3-8B (fp16) on one A10G-class 24 GB GPU, a persistent
...
- **`qwen3-8b-fp16`** (default): `Qwen/Qwen3-8B`, fp16, `A10Gx8x32` (24 GiB),
...
## Follow-up execution plan (not this slice)

The live deployment and the real cold-start measurement run afterward as an
**explicit, customer-visible step** with the customer's Baseten credentials.
Nothing in `baseten_demo` creates, wakes, or bills a real deployment when run
from this repository without explicit server-side credentials.

1. **Create the deployment.** Push the Truss definition produced by
   `build_config_yaml(profile)` (`baseten model push`, or the Management API
   deployment endpoint), then apply the autoscaling payload from
   `build_autoscaling_settings(profile)` via
   `PATCH .../autoscaling_settings` (`min_replica=0`, `max_replica=1`,
   `scale_down_delay=300`). Record the returned `model_id` / `deployment_id`.
2. **Record the cold-start measurement.** Export server-side
...
3. **Attach an 8B-matched intake adapter** (a follow-up on the customer's
   WSL2 machine — the existing 1.7B/4B adapters are dimensionally
   incompatible) and re-measure with the adapter-load path.
4. **Qwen3-14B benchmarking** via the `qwen3-14b` profile.
5. **clinicalinterview.com demo page** calling this backend (UI is a separate
   slice; the backend seam is what a future "Start Demo" button calls).
6. **aLoRA** base-KV-cache branching stays on the
