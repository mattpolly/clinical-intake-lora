...
- [ ] 2.1 Create `baseten_demo/profiles.yaml` with the default
  `qwen3-8b-fp16` profile (A10G-class 24 GB, min_replicas=0, max_replicas=1,
  idle timeout configurable 2–5 min, adapter slots as a list) and a
  representable `qwen3-14b` profile
- [ ] 2.2 Implement profile loading/validation (config-only profile
  switching)
...
- [ ] 3.5 Implement bounded failure behavior: at most one retry, explicit
  error states, error instrumentation events
...
- [ ] 4.1 Implement JSON-lines event writer under `outputs/baseten_demo/`
  ...
  inference_result, last_activity, scaled_to_zero best-effort)
- [ ] 4.2 Ensure events are bounded and secret-free (key-scan test)

## 5. Tests and evidence

- [ ] 5.1 Unit tests: profile loading/validation, deployment definition
  builder, event writer
